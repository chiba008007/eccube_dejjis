...
const mockResponseXml = fs.readFileSync(path.join(__dirname,'../../server/mockdata/mock-cxml-api-response-PunchoutOrderMessage_finCatalog.xml' ), 'utf-8');
...
test.beforeEach(async ()=>{
  server = createServer((req, res)=>{
    if(req.method === 'POST' && req.url === '/punchOutSetupRequest3_finCatalog'){
...
test('ユーザーがAmazonビジネス上で「カタログ連携を終了してサインアウト」を押下したケース(punchOutSetupRequest3_finCatalog)', async () => {
  ...
  expect(xmlText).not.toContain('<SupplierPartAuxiliaryID>'); // 含まれていないこと

...
const mockResponseXml = fs.readFileSync(path.join(__dirname,'../../server/mockdata/mock-cxml-api-response-PunchoutOrderMessage.xml' ), 'utf-8');
...
test('モックAPIにPOSTして固定レスポンスを返す_PunchoutOrderMessage(cXML)', async () => {
  const context = await playwrightRequest.newContext();
  const response = await context.post(`${host}:${port}/amazonApiSample`, {
    headers: {

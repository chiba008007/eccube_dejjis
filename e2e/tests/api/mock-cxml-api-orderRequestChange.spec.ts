...
const mockResponseXml = fs.readFileSync(path.join(__dirname,'../../server/mockdata/mock-cxml-api-orderConfirmation_change.xml' ), 'utf-8');
...
test.beforeEach(async ()=>{
  server = createServer((req, res)=>{
    if(req.method === 'POST' && req.url === '/orderRequestChange'){
...
test('確定した注文金額情報の確認（商品価格変動時）。1注文に対して注文後24時間以内に1回送信される(orderRequestChange)', async () => {
  const context = await playwrightRequest.newContext();
  const response = await context.post(`${host}:${port}/orderRequestChange`, {
    headers: {
  ...
  expect(xmlText).toMatch(/<UnitPrice[\s\S]*?<\/UnitPrice>/);
